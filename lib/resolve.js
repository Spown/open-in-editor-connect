...
module.exports = function resolve (filePath, options) {
  var opts = options
  var UP_PATH_REGEXP = /(?:^|[\\/])\.\.(?:[\\/]|$)/
  var absRoot = path.resolve(opts.root)
  var filename, pathParts, transformMatches

  if (options.transform instanceof RegExp && (transformMatches = filePath.match(options.transform))) {
    filename = transformMatches.slice(1).join('')
  } else {
    filename = filePath
  }
